/*
 * Módulo de ejemplo para Apache 2.4
 *
...
typedef struct
{
	...
	bool flush; // Permite o no visualizar el fichero de logins completo (p.e. en /auth/flush).
} auth_ex_cfg;
...
static const command_rec directives[] =
{
	 ...
	 * que acepta 1 parámetro, la función que lo maneja es 'set_logins_path', a NULL
...
module AP_MODULE_DECLARE_DATA auth_example_module = {
    ...
    NULL, // Función que creará una nueva estructura 'auth_ex_cfg' (para servidores distintos).
...
static int auth_example_handler(request_rec *r)
{
     ...
     * funciones comunes de los sistemas operativos. Si los paths no apuntan a un fichero o apuntan
 ...
 * si tenemos una localización '/auth' y otra '/auth/flush', al crear la configuración
 * de '/auth/flush' Apache creará primero la configuración de '/auth' (leyendo del fichero
 * .conf), después la de '/auth/flush', y nos pasará punteros a ambas estructuras de configuración
